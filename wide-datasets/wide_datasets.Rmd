...
The tricky part here is, if we were to use **tidyjson** on the root node of this JSON document, we wouldn't get the information we need.  

What we actually need is the fourth node "data", which itself is a series of nested JSON nodes.  We'll use the **enter_object** and **gather_object** functions to extract the content of this "data" node, **spread_all** to create a table of all the json elements, and then convert back into a standard R dataframe:
